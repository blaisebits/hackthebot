...
def get_exploit_step_status_template()->ChatPromptTemplate:
    ...
    system_template = ("# Role\n"
                        ...
                        "* If the <STEP> did not completed successfully use the following criteria to update the steps array.\n"
                        "  * Never return an empty `ExploitStepUpdates`, it must always contain the complete dataset when called\n"
                        "    * If returning a blank `ExploitStepUpdates` you must explain why it is blank\n"
                        "  * Always increment the current <STEP> `iterations` field by 1\n"
                        "  * Analyze the <STEP> `scratchpad` for historical attempts to avoid trying previously failed revisions\n"
                        "  * You may modify, add, or delete steps\n"
                        "    * If the step needs to be modified then provide a revised `step_task` that corrects for the error or blockage\n"
                        "    * If a new step is needed to bypass the blockage, create a new step and insert it into the steps array\n"
                        "    * If the a step in the array is no longer needed after revisions it can be removed from the returned output\n"
