...
def get_exploit_step_status_template()->ChatPromptTemplate:
    system_template = ("You are a world class exploit analyzer. Use the following criteria to determine the values"
                       ...
                       "When determining the `ExploitStepStatus` `status` value, only consider information from the <OUTPUT>.\n"
                       "For determining `Create initial access exploit to the target` the task's the output should be complete and functional code or "
                       "a structured tool call with the `name` and `args`"
                       "* Initial Access Exploits may contain placeholders values like <command>, this is to be considered valid."
                       ...
                       "producing observable effects such as outbound network callbacks, file/process changes, or DOM/script actions.\n"
                        ...
                        "* Consider the nature of the blockage, if the revisions needs to occur before the current task, set insert_step as True\n"
                        "* <TASKS> may contain place holder text like [filename].php, these should not be considered hard requirements for completion\n"
                        ...
                        "  * Example of valid revision: \"Rename the PHP web shell to use a .jpg extension.\"\n"
                        "  * Example of invalid revision: \"Rename the PHP web shell to use a .jpg extension or try a different format.\" (This uses 'or' and is compound.)\n"
                        ...
                        # "* Revisions should be a single atomic instruction\n"
                        # "  * Do not under any circumstance use the words `or` and `and` in the Revision field.\n"
                        # "* Revisions should never be compound instructions, see the examples below:\n"
                        ...
                        # "  * Bad Example(Compound OR/AND statement): Rename the file to use a bypass extension like .xxx or .yyy and upload it\n"
